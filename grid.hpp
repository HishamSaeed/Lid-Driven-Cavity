...
public:
    ...
    // Added method for acquiring velocity as 2D array
    void velocity(double** vec, velocity_type type);

    // Get and Set Pressure
    void pressure(matrix<double>& vec);
    void set_pressure(matrix<double>& vec);
    // Added method for acquiring pressure as 2D array
    void pressure(double** vec);
