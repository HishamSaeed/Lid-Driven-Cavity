...
// -----------------------------------Method Added To Skeleton----------------------------------------------
void Grid::velocity(double** vec, velocity_type type){
    if (!(type == velocity_type::U || type == velocity_type::V)) {
        std::cerr << "Wrong velocity type" << std::endl;
    }
    else {
        ...
        for(int x=0; x < Grid::imaxb();x++) {
            for (int y=0; y < Grid::jmaxb(); y++) {
                // Accessing velocity
                vec[x][y] = _cells.at(x).at(y).velocity(type);
            }
        }
    }
}
...
// -----------------------------------Method Added To Skeleton----------------------------------------------
void Grid::pressure(double** vec){
     // Iterate over cells
    for(int x=0; x < Grid::imaxb();x++) {
        for (int y=0; y < Grid::jmaxb(); y++) {
            // Accessing velocity
            vec[x][y] = _cells.at(x).at(y).pressure();
        }
    }
}
// ---------------------------------------------------------------------------------------------------------
...
void Grid::print_pressure() {
    for(int y=Grid::jmaxb()-1;y >= 0;y--) {
        for(int x=0;x<Grid::imaxb();x++){
                // Accessing pressure
                std::cout << std::fixed << std::setprecision(7) << std::right << std::setw(2) << Grid::_cells.at(x).at(y).pressure() << " ";
            }
            // Print new line
            std::cout << std::endl;
        }
